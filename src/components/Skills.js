...
function Skills() {
  ...
  const skills = [
    ...
    { name: 'C', Icon: COriginal, proficiency: '95%' },
  ...
  return (
    <section id='skills' className='bg-bg1 my-10' style={{ background: 'black' }}>
      <h2 className='text-textGreen text-3xl font-bold text-center mb-10' style={{ color: '#306dbc', fontWeight: '700' }}>
        Skills
      </h2>
  ...
  if (!skill.Icon) {
    console.error(`Icon for skill ${skill.name} is undefined. Check your imports.`);
    return null;
  }
  ...
  return (
    ...
    >
      ...
      {/* Icon */}
      <IconComponent
        size={iconSize || 100} // Fallback size if `iconSize` is undefined
        className="w-full h-full py-5"
      />
      
      {/* Skill Name */}
      <span
        className="text-md text-textGreen mt-24"
        style={{ color: '#306dbc', fontWeight: '600' }}
      >
        {skill.name}
      </span>
    </div>
